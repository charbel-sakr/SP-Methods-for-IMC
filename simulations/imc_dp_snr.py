...
def evaluate_analog_DP(X,W,Co):
    ...
    return (NDP/Vdd)*numerator/denominator
...
def evaluate_snr_dp(Nsample,NDP,BADCs,BX,BW,Co):
    SQNR_simulated_FR = []
    SQNR_simulated_LM = []
    SQNR_simulated_OCC = []
    ...
    rho2 = 4.14e-21
    rho3 = 6.01e-33
    sig2eta = noise_factor*NDP*(0.25*rho1/Co + rho2/Co + rho3/np.square(Co))
    print(sig2eta)
    print(10*np.log10(varY/(sig2wy+sig2xy+sig2eta)))
    ...
    for (BADC,zeta_occ,sigOCC,sigLM) in zip(BADCs,zetas,sigOCCs,sigLMs):
        ...
        for bx in range(BX):
            ...
            for bw in range(BW):
                ...
                Yba = evaluate_analog_DP(Xbs[bx],Wbs[bw],Co)
                Yb_FR = quantize_adc_full_range(Yba,NDP,BADC)
                #py = 0.25
                muYb = np.mean(Yba)#NDP*py
                varYb = np.var(Yba)#NDP*py*(1-py)
                Yb_OCC = muYb+np.sqrt(varYb)*custom_quantization((Yba-muYb)/np.sqrt(varYb),occ_levels)
                Yb_LM = muYb+np.sqrt(varYb)*custom_quantization((Yba-muYb)/np.sqrt(varYb),LM_levels)
                Ybw_FR.append(Yb_FR)
        ...
        SQNR_formula_FR.append(10*np.log10(varY/(sig2wy+sig2xy+sig2eta+sigqY)))
        SQNR_formula_LM.append(10*np.log10(varY/(sig2wy+sig2xy+sig2eta+slice_factor*sigLM)))
        SQNR_formula_OCC.append(10*np.log10(varY/(sig2wy+sig2xy+sig2eta+slice_factor*sigOCC)))
...
SQNR_simulated_FR,SQNR_simulated_OCC,SQNR_simulated_LM,SQNR_formula_FR,SQNR_formula_OCC,SQNR_formula_LM = evaluate_snr_dp(Nsample,NDP,BADCs,BX,BW,Co)
