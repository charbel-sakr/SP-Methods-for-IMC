    ...
    for BS in BSs:
        ...
        for bw in range(BW):
            ...
            for i in range(1,NFS+1):
                ...
                Yi = evaluate_analog_DP(Xi,Wb,BS)
            ...
            if BI !=0:
                ...
                YIS = evaluate_analog_DP(XIS,Wb,BI)
...
def predict_sqnr(NDP,BADC,BX,BW):
    ...
    rho2 = 4.14e-21
    rho3 = 6.01e-33
    sig2_eta_base = 1.5*NDP*(1-np.power(4.0,-BX))*(1-np.power(4.0,-BW))/3.0
    ...
    sig2xy = np.square(deltaX)*NDP*varW/12
    sig2wy = np.square(deltaW)*NDP*EX2/12
    ...
    for BS in BSs:
        ...
        sig2_eta_slice = rho2/Co + rho3/np.square(Co) + np.true_divide(rho1*(2.0-np.power(2.0,-BS)),12*(1.0-np.power(2.0,-BS))*Co)#0.25*rho1/Co#
        sig2_eta_total = sig2_eta_base*sig2_eta_slice#/(1-np.power(4.0,-BS))
        SQNR_linear = np.true_divide(varY,sig2xy+sig2wy+sig2ADC_base*beta+sig2_eta_total)
...
Nsample=10000
BADC = 3
BXs= [8,10]#np.arange(8,17,4).tolist()
BW=4
...
for BX in BXs[::-1]:
    ...
    line_handles.append(line11)
    line_handles.append(line1)
#plt.legend(handles=line_handles,bbox_to_anchor=(0, 2), loc='upper left',fontsize=20,ncol=3)
...
plt.savefig('SNR_vs_BS.pdf',bbox_inches='tight')
