...
def evaluate_sqnr(Nsample,NDP,BADCs,BX,BW):
    ...
    Y = evaluate_DP(X,W)
    Xq = quantizeUnsigned(X,BX)
    Wq = quantizeSigned(W,BW)
    Yq = evaluate_DP(Xq,Wq)
    ...
    print(10*np.log10(varY/(sig2wy+sig2xy)))
    ...
    for (BADC,zeta_occ,sigOCC,sigLM) in zip(BADCs,zetas,sigOCCs,sigLMs):
        ...
        Y_LM = np.sqrt(varY)*custom_quantization(Yq/np.sqrt(varY),LM_levels)
        occ_levels = zeta_occ*placeUniformLevels(M)
        Y_OCC = np.sqrt(varY)*custom_quantization(Yq/np.sqrt(varY),occ_levels)
        Y_FR = NDP*quantizeSigned(Yq/NDP,BADC)
        ...
        deltaY = NDP*np.power(2.0,1-BADC)
        sigqY = np.square(deltaY)/12.0
        SQNR_formula_FR.append(10*np.log10(varY/(sig2wy+sig2xy+sigqY)))
        SQNR_formula_LM.append(10*np.log10(varY/(sig2wy+sig2xy+varY*sigLM)))
        SQNR_formula_OCC.append(10*np.log10(varY/(sig2wy+sig2xy+varY*sigOCC)))
...
ax.set_xlabel(r'$B_Y$',fontsize=30)
ax.set_ylabel(r'SQNR (dB)',fontsize=30)
#ax.set_yscale('log')
ax.tick_params(axis='both',labelsize=20)
ax.grid(axis='both',which='both')
ax.set_ylim(bottom=0)
plt.savefig('SQNR_vs_BY.svg',bbox_inches='tight')