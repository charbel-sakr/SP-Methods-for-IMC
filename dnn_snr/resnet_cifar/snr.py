...
def evaluate_sqnr(Nsample,NDP,BADCs,BX,BW):
    ...
    X = np.transpose(np.load('activations.npy'))
    #print(np.min(X))
    #print(np.max(X))
        ...
        for bw in range(BW):
            ...
            if zeta_right==0:
                ...
                if delta_zeta>=M:
                    ...
                    s =  0.25 if BADC>5 else 0.185 if BADC ==5 else 0.6 if BADC==4 else 0.5
...
plt.legend(handles=line_handles,loc='lower right',fontsize=20,ncol=1)
